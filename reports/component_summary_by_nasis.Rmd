...
s <- site(comp)
h <- horizons(comp)# parent material

# component

kable(with(s, data.frame(
  dmudesc = abbreviate(dmudesc, 20), 
  compname, comppct_r, majcompflag, localphase, drainagecl, 
  slope_l, slope_r, slope_h,
  taxpartsize, taxsubgrp))
  )

# parent material and landform
kable(with(s, data.frame(dmudesc = abbreviate(dmudesc, 20), compname, pmgroupname, landform.string)))
```
...
h <- merge(vars_slice, site(comp)[c("dmuiid", "coiid", "compname", "comppct_r")], by = "coiid", all.x = TRUE)
...
gg_comp <- function(x, var) {
  ...
  xlim(200, 0) +
  xlab("depth (cm)") +
  facet_grid(paste("dmu-", dmuiid) ~ paste(compname, comppct_r, "%")) + 
  coord_flip() +
  ggtitle(var)
  }
