...
source("C:/workspace/github/aqp/trunk/R/SoilProfileCollection-methods.R")
...
lh <- merge(lh, h[vars], by = c("labsampnum", "hzdept", "hzdepb"), all.x = TRUE)
lh$total_frags_pct <- ifelse(is.na(lh$total_frags_pct), lh$total_frags_pct_cal, lh$total_frags_pct)
lh$total_frags_pct_cal <- NULL
...
lh <- transform(lh,
                ...
                awc = round((wthirdbarclod - wfifteenbar) * dbthirdbar * 
...
genhz_med <- sort(with(lh, tapply(hzdepm, genhz_nlcd, median)))
lh$genhz_nlcd <- factor(lh$genhz_nlcd, labels = names(genhz_med))
