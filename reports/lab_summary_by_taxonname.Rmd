...
lh <- merge(lh, h[vars], by = c("labsampnum", "hzdept", "hzdepb"), all.x = TRUE, sort = FALSE)
...
lh$hzname <- ifelse(is.na(lh$hzname), "missing", lh$hzname)
lh <- transform(lh,
...
genhz_med <- names(sort(with(lh, tapply(hzdepm, genhz, median))))
lh$genhz <- factor(lh$genhz, levels = genhz_med, ordered = TRUE)
...
genhz_med <- names(sort(with(h, tapply(hzdepm, genhz, median))))
h$genhz <- factor(h$genhz, levels = genhz_med, ordered = TRUE)
