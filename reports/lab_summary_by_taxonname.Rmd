...
source('C:/workspace/github/soilDB/trunk/R/get_site_data_from_NASIS_db.R')
source('C:/workspace/github/soilDB/trunk/R/fetchNASIS.R')
source('C:/workspace/github/soilDB/trunk/R/utils.R')
...
names(lh) <- gsub("measured", "", names(lh))

lh$hzname <- ifelse(is.na(lh$hzname), lh$hznameoriginal, lh$hzname)
lh <- transform(lh,
                hzname = ifelse(is.na(hzname), "missing", hzname),
                genhz = generalize.hz(hzname, ghr$n, ghr$p),
                hzdepm = (hzdept + hzdepb) / 2,
                cec7clay = round(cec7 / (claytot - claycarb), 2),
                organicmatpct = carbonorganicpct * 2
                )
genhz_med <- sort(with(lh, tapply(hzdepm, genhz, median)))
lh$genhz <- factor(lh$genhz, levels = names(genhz_med), ordered = TRUE)
horizons(l) <- lh
...
genhz_med <- sort(with(h, tapply(hzdepm, genhz, median)))
h$genhz <- factor(h$genhz, levels = names(genhz_med), ordered = TRUE)
horizons(f) <- h

h <- data.frame(lapply(h, na_replace))
s$coverkind_1 <- as.character(s$coverkind_1)

lh <- merge(lh, lp[, c("peiid", "labpeiid")], by = "labpeiid", all.x = TRUE)
lh <- merge(lh, s[, c("peiid", "coverkind_1")], by = "peiid", all.y = TRUE)
...
```{r, fig.width=8}
# Summarize site data
lp.sub <- lp[, c("noncarbclaywtavg", "claytotwtavg", "le0to100", "wf0175wtavgpsc", "volfractgt2wtavg", "cec7clayratiowtavg")]
...
lh$genhz_nlcd <- as.factor(lh$genhz_nlcd)
