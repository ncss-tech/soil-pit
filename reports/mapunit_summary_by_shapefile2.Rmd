...
if (!file.exists(cache)){
  ...
  test <- by(mupolygon, mupolygon$idx, function(x) st_sample(x, 10))
  test2 <- lapply(test[[1]], function(x) data.frame(x))
  test2 <- do.call(rbind, test)
  apply(mupolygon, 1, function(x) st_sample(x, 10))
  
  mupolygon.s <- st_sample(mupolygon, size = n)
  
  
  mupolygon_sp <- as(mupolygon.s, "Spatial")

  geodata <- raster_extract(mupolygon_sp)
  idx  <- st_intersects(mupolygon.s, mupolygon)
