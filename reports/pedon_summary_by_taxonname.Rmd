...
srf <- mutate(srf, 
              total_srf = gravel + cobbles + stones + 
                boulders + flagstones + channers,
...
if (sum(srf$total_srf, na.rm = T) != 0) {
  bwplot(variable ~ variable, data = srf.lo, 
...
num_vars <- names(h)[! names(h) %in% c('peiid', 'phiid', 'hzname', 'genhz', 'hzdept', 'hzdepb', 'labsampnum', 'd_r', 'd_g', 'd_b', 'm_r', 'm_g', 'm_b', 'parafine_gravel', 'paragravel', 'paracobbles', 'parastones', 'paraboulders', 'parachanners', 'paraflagstones', 'unspecified') &
                       sapply(h, is.numeric) & 
