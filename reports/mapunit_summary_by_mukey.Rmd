...
mukey <- c('161480', '161880') # List of mukey
...
gdal_setInstallation(search_path = gdalpath, rescan=TRUE)
...
dsn <- paste0(office_folder, "cache/", shp, ".shp")
cache_shp <- paste0(office_folder, "cache/", shp, ".shp")
...
if (!file.exists(cache)){
  ogr2ogr(
    ...
    dst_datasource_name = cache_shp,
    layer = "MUPOLYGON",
    where = paste0("MUKEY IN (", noquote(paste0("'", mukey, "'", collapse=",")),")"),
...
pol.lo1$ssa_musym <- factor(pol.lo1$ssa_musym, levels = rev(unique(pol.lo1$ssa_musym)))
                                                          
bwplot(ssa_musym ~ value | variable, data = pol.lo1, 
       main = "Boxplots of polygon metrics", 
...
kable(prop.table(xtabs(~ ssa_musym + slope_classes, data = data, drop.unused.levels = TRUE), margin = 1) * 100, align = "c", digits=0, caption="Slope classes")
...
lulc_t <- addmargins(prop.table(xtabs(~ ssa_musym + lulc_classes, data = data, drop.unused.levels = TRUE), margin = 1)) * 100
...
data.lo$ssa_musym <- factor(data.lo$ssa_musym, levels = rev(unique(data.lo$ssa_musym)))

bwplot(ssa_musym ~ value|variable, data=data.lo, 
       main = "Boxplots of map unit properties", 
       scales=list(x="free"), axis = axis.grid,
