...
mukey <- read.csv("C:/Users/stephen.roecker/Documents/test.csv")
mukey <- as.character(mukey$Rec.ID)
mukey <- data.frame(mukey=mukey, stringsAsFactors=FALSE)
test <- read.dbf("M:/geodata/project_data/11REGION/muaggatt.dbf")
test$mukey <- as.character(test$mukey)

test2 <- merge(test, mukey, by.y="mukey")
paste("(", noquote(paste("'", test2$mukey, "'", collapse=",", sep="")),")", sep="")
```
...
ind <- readOGR(dsn="M:/geodata/soils/Region_11_Soils_SSURGO_FY2015/MLRA_11_IND_FY15.gdb", layer="chtext", encoding="OpenFileGDB")
