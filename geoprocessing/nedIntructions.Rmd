...
Next numerous parameters need to be set which get used later by many of the functions or commands. Modify these file paths and lists as necessary. For example, I organized my files by "M:/geodata/project_data/11ATL"", so 11 will have to replace by 10 or 2 for your respective Regions.
```{r, eval=FALSE}
# Set parameters
...
dsn <- paste("M:/geodata/project_data/11IND/RTSD_Region_11_IND_FY15.gdb")
layer <- "test3"
...
```{r, eval=FALSE}
tiles <- "M:/geodata/elevation/ned/tiles"
...
```{r, eval=FALSE}
nlcd.l <- list(nlcd.p)
dst.p <- "M:/geodata/project_data/REGION11/nlcd30m_R11.tif"
mosaicNlcdList(nlcd.l, dst.p)

dem.l <- list(paste(pd.p, office.l, "/ned30m_11", office.l, ".tif", sep=""))
dem.p <- "M:/geodata/project_data/Region11/ned30m_R11.tif"
