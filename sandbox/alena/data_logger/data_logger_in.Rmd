...
map('county', 'ohio') # plot county boundaries for all of Indiana
map('county', 'ohio,madison', add=T, fill = T, col="purple") #add Pulaski County, Indiana in purple to map
map.text("county", "ohio,Madison", add=T) #add text for Pulaski

#read all the csv files from a folder as one table
datalogger=ldply(list.files(path="E:/geodata/project_data/11IND//IPO_Data_Loggers/data_loggers_2017/kokomo_71_Madison_OH",pattern="csv",full.names=TRUE),function(filename) {
...
datalogger$convin <- (21-datalogger$inches) #inches -
...
inyear <- aggregate(convin ~ year, data = datalogger, mean) #averages years in inches
inmoyear <- aggregate(convin ~ month+year, data = datalogger, mean) #averages years in inches
