...
sp.C <- C[complete.cases(C),]

#xy cooridanates
xy <-sp.C[,3:4]

C.sp <- SpatialPointsDataFrame(coords = xy, data = sp.C,
        proj4string = CRS("+proj=longlat +datum=WGS84 +init=epsg:5070"))

# 
# #from https://github.com/smroecker/raca/blob/master/stephen/explore_stocks.Rmd
...
# 
# spplot(C.sp, zcol = "SET", main = "SOC Pedon Stocks",
#        col.regions = cols, cex = 0.5, alpha = 0.5,
#        as.table = TRUE, layout = c(2, 2), sp.layout = list(us_sp), scales = list(draw = TRUE),
#        key.space = list(
#          space = "right",
...
plot(C.sp) # this doesn not look right - I need to check the projection of the NWCA points
plot(C.sp)[C.sp$SET=='RaCA']
