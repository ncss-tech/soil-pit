...
combo_pedcalc <- combo %>% mutate(BDmix = ifelse(!is.na(BD), BD, BD.RF), thick = Bottom - Top)

#SOCdensity
combo_pedcalc <- combo %>% mutate(SOCden = BD*SOC/100, SOCden_mix = BDmix*SOC/100, SOCden_MODEL = BD.RF*SOC/100) 


#SOC density with course fragments
  combo_pedcalc <- combo_pedcalc %>% mutate(SOCden_CF = SOC*BD*(1 - ifelse(CoarseFrag == 0, 0, CoarseFrag/100)),                                                                        SOCden_CF_MODEL = SOC*BD.RF*(1 - ifelse(CoarseFrag == 0, 0, CoarseFrag/100)),                                                               SOCden_CF_mix = SOC*BDmix*(1 - ifelse(CoarseFrag == 0, 0, CoarseFrag/100))
                                             )
...
combo_pedcalc <- combo_pedcalc %>% mutate(thick = BOT - TOP) 
# 
