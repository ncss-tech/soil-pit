...
tn <- tnsh %>%
  select(-(matches("X"))) %>%
   gather(Prop, Value, -c(KSSL.Project.lab, KSSL.Project.name, UserPedonID, labsampno, layerID, Comp, Cover_kind, Field, Crop,  Mgmt, Soil, Site, hor_desg, hor_top, hor_bot)) %>%
...
tn %>%
  select(hor_desg, hor_top, hor_bot, Field, Comp, Prop, Value, Cover_kind) %>%
  filter(str_detect(Prop, "BD")) %>%
...
bd <- tn %>%
  select(KSSL.Project.lab, KSSL.Project.name, UserPedonID, labsampno, layerID, Comp, Cover_kind, Crop,  Mgmt, Soil, Field, hor_desg, hor_top, hor_bot, Prop, Value) %>%
  filter(str_detect(Prop, "BD")) %>%
  spread(Prop, Value) 
...
bd2 <- bd %>%
  left_join(
    tn %>% 
        select(KSSL.Project.lab, KSSL.Project.name, UserPedonID, labsampno, layerID, Field, Comp, Cover_kind, Crop,  Mgmt, Soil, hor_desg, hor_top, hor_bot, Prop, Value)%>% 
...
# 
 ...
 ################################# Rtools and nlme don't want to load with R 3.4.0
# #nlme: Linear and Nonlinear Mixed Effects Models
#  library(nmle)
# 
# #fit model; treat Soil as a block since we aren't tryin to compare them; fields are inlcuded to correctly deal with pedons that are sub-samples of the replicate 
#  
# fit_lmod <- function(data){
#   lmod <- lme(Value ~ Comp + Soil, random = ~1|Field) 
#   return(anova(lmod))
# }
# 
# fit_lmod(surf.n$data[[1]])
###############################
...
fit_lmer <- function(data){
  fit <- lmer(Value ~ Comp + 1|Soil/Field, data = data)
  fit2 <- lmer(Value ~  1|Soil/Field, data = data)
    return(aov(fit,fit2))
