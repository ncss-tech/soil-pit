...
#POX (AC) sample pool used to select samples for further analysis
DSP_url <- 'https://nrcs.box.com/shared/static/e53czbbenk8cjfg4tp4hmvxtm2vnprn4.xlsx'
...
dsp %>%
  select(Name, DSP.Project, UserPedonID, LabPedonNo, labsampleno, dsp_limslayer_id, Soil, Comparison, COND, PlotID, hor_desg, Hor_sequ, Comp_layer, hor_top, hor_bott, AggStab, Bgluc, Pom_C, Pom_N, Pom_S, POX_C, Tot_C, Tot_N, Tot_S, CaCarb ,ph_h20, Clay, BulkDensity, SOC, TN)
...
dsp %>% filter(Hor_sequ)
  ggplot(aes=Tot_C, y = Name, fill = factor(...quantile...))) +
  stat_density_ridges(geom="density_ridges_gradient", calc_ecdf = TRUE, quantiles = c(0.25, 0.75)) +
  scale_fill_manual(name = "Probability", labels = c(" 0 - 25%", "25 = 75%", "75 - 100%"))
