...
group_by(test, region) %>% filter(projecttypename == "SDJR" & fy == "2016" & ) %>% summarize(
  ...
  correlated_acres = sum(new_muacres, na.rm = TRUE),
  pct_complete_acres = round((correlated_acres / approved_acres) * 100)) %>% 
