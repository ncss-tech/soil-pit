...
source("C:/workspace/soil-pit.git/trunk/queries/kssl_queries.R")
...
test <- sdjr_correlation(asymbol, "2016%25", start_date, finish_date)

group_by(test, region) %>% filter(spatial_change == TRUE) %>% summarize(n_projects = length(unique(projectiid))) %>% arrange(desc(n_projects))

group_by(test, region) %>% filter(spatial_change == TRUE) %>% summarize(n_musym = length(spatial_change)) %>% arrange(desc(n_musym))
