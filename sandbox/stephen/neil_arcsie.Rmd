...
The classification tree of the FD classes using only the DEM derivatives as predictors found the best split using a slope gradient of 6 percent, and had an overall accuracy of `r round(cm3$overall[1], 2)`. This tree however was only able to map classes 1 and 3. The split at a slope of 6 percent matches the matches the original slope break used within Morrow (OH117) and Knox (OH083) Counties.  
...
The classification tree of the clusters using only the DEM derivatives as predictors found the best split using across slope (PlanCrv) curvature, and had an overall accuracy of `r round(cm4$overall[1], 2)`.
