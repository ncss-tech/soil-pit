...
A fundamental (and labor intensive) step in the mapping process is how to aggregate horizons from field and lab pedons into component horizons. Historically this has been done with pencil and paper. A practice that still continues today for a variety of reasons, including:
...
To generate reports in R we use RStudio which is an Integrated Development Environment (IDE) for R. It provides a minimalist Graphical User Interface (GUI) and arranges various features among-st four windows. Prior to importing data from NASIS it is necessary to install several R packages and their dependencies (Roecker and Stephens, 2015), and setup an ODBC connection to NASIS (Beaudette, 2015). These steps are fully documented online, and readers are pointed to them for full details. R is a highly extendable environment, and installing additional packages is common place. 
...
Once a lookup table has been developed, pattern matching is used to develop a new generic HD for each horizon. This uses functions designed to parse the text from each HD, and match it to our new generic HD. It search for any combination of characters before or after the given pattern. Special characters are used to special metacharacters caret "^" and dollar sign "\$" serve as anchors or anti-wildcards for the beginning and end of the given pattern respectively. For example, the HD "Bt" will match any permutation of Bt, such 2Bt or Bt1. To exclude 2Bt horizons, we would use "^Bt". Conversely to exclude Bt1 horizons, would would use "Bt\$".
