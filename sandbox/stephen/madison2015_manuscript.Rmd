...
Once a lookup table has been developed, pattern matching is used to develop a new generic HD for each horizon. This uses functions designed to parse the text from each HD, and match it to our new generic HD. It search for any combination of characters before or after the given pattern. Special metacharacters serve as anchors or anti-wildcards for the beginning (i.e. caret "^"), and end (i.e. dollar sign "\$") of the given pattern. For example, the HD "Bt" will match any permutation of Bt, such 2Bt or Bt1. To exclude 2Bt horizons, we would use "^Bt". Conversely to exclude Bt1 horizons, would would use "Bt\$". This information is stored in a text file, and referenced by the report. If the user is satified with the resulting HD, they can upload it to the comp layer id in NASIS for future use.

- GHL and rules for the aqp loafercreek sample dataset:
  - **A**: `^A$|Ad|Ap`
  - **Bt1**: `Bt1$`
  - **Bt2**: `^Bt2$`
  ...
  In order read NASIS data into R, a user must first load their seleted set with the Site, Pedon, and NCSS Pedon Lab Data tables. A selected set is a view or virtual table that is created via a query, and serves as a working subset of a user's local NASIS database. NASIS has numerous queries to accomplish this. Once loaded the soilDB is designed to automatically load the Site and Pedon tables using the following command.
