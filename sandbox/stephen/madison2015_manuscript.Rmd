...
kable(addmargins(table(miami_field$genhz, miami_field$hzname)), digits = 0, caption = "Horizon designations vs generic horizon designations (counts)")
...
The tables and figures shown above, and the online examples demonstrate an attempt to streamline the aggregation of point data. Prior to the advent of NASIS their were many early attempts at estimate the RIC for soil properties (Young etal., 1991; Jansen and Arnold, 1976). These earlier attempts looked at estimates for portions of the soil profile, such as surface texture or subsoil clay content (not necessary GHL), and preferred parameteric estimates (i.e. mean and confidence intervals). They also clearly demonstated the disconnect between the limits for taxonomic units and map unit components. This secondary issue is now addressed by Soil Survey Technical Note 4 (USDA, 2003), which allows map unit components to L and H values to range beyond that of a soil series taxonomic boundaries.
