...
h.lo$variable <- factor(h.lo$variable, levels=c('clay', 'phfield'), labels=c('Field Est. Clay (%)', 'Field Est. pH'))
lh.lo$variable <- factor(lh.lo$variable, levels=c('claytot', 'ph1to1h2o'), labels=c('Lab Meas. Clay (%)', 'Lab Meas. pH (1:1 H2O)'))

kable(lh.c, align = "c", caption = "Summary of Miami lab pedons by generic horizon (min, 25th, median, 75th, max)(n)")

qplot(genhz, value, data = h.lo, geom = "boxplot", ylab='', xlab = "Generalized Horizon Label", main = "Boxplot of Miami field pedons by generic horizon") + facet_wrap(~variable, scales = "free") + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))

qplot(genhz, value, data = lh.lo, geom = "boxplot", ylab='', xlab = "Generalized Horizon Label", main = "Boxplot of Miami lab pedons by generic horizon") + facet_wrap(~variable, scales = "free", ncol = 4) + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
