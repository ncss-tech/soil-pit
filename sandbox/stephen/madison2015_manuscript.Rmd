...
As an example the following tables and figures show a excerpts of all the field and lab data labeled as the Miami soil series with NASIS.

The lab data in NASIS is primarily composed of numeric data, therefore it is easily summarized by percentiles and boxplots.
...
title("Sample of soil profile plot of field pedons")
...
legend('bottomleft', legend=hz.names, pt.bg=c(cols), pch=22)
title("Sample of soil profile plot of lab pedons")
...
Rarely can soil scientists observe full pedons or polypedons in the field due to the limitations of human observation. Therefore we can only approximate the range of their internal variability by examining several soil profiles and comparing multiple descriptions. The purpose of this paper to a field and lab report to rapidly analyze such data.
