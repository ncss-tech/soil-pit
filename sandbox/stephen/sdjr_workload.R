...
goals <- read.csv("region11_goals.csv", stringsAsFactors=F)
projects <- str_split(goals$Project.Name, pattern=" - ")
goals$mapunits <- unlist(lapply(projects, function(x) x[3]))
goals$components <- unlist(lapply(str_split(goals$mapunits, " "), function(x) x[1]))

jue <- subset(goals, SS.Office == "11-JUE")
jue$components <- unlist(lapply(str_split(jue$mapunits, " "), function(x) x[2]))

test <- ddply(goals, .(SS.Office), summarise, n_projects = length(unique(Project.Name)), n_mapunits = length(unique(mapunits)), n_majors = length(unique(unlist(lapply(str_split(components, "-"), function(x) x[1])))))
test2 <- ddply(jue, .(SS.Office), summarise, n_projects = length(unique(Project.Name)), n_mapunits = length(unique(mapunits)), n_majors = length(unique(unlist(lapply(str_split(components, "-"), function(x) x[1])))))
test[7, ] <- test2
...
test[12, ] <- c("Average", round(apply(test[2:8], 2, mean)), 0)
