...
test <- ddply(offices, .(office, AREASYMBOL), summarize, n_musym = length(strsplit(paste0(sort(unique(MUSYM)), collapse = ", "), ", ")[[1]]), musym = paste0(sort(unique(MUSYM)), collapse = ", "))

test2 <- join(test, region, by = "AREASYMBOL")
test3 <- subset(test2, Region == 11, select = c("office", "AREASYMBOL", "n_musym", "musym"))

test4 <- ddply(test3, .(AREASYMBOL), summarize, offices = paste0(office, collapse = ", "))
test5 <- join(test3, test4, by = "AREASYMBOL")
test5 <- cbind(test5[c(1:3)], dom = "", test5[c(5, 4)])
...
write.csv(test5, paste0("test", format(Sys.time(), "%Y_%m_%d"), ".csv"))
