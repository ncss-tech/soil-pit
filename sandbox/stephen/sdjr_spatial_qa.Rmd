...
region <- 11
office <- "11-MAN"
mupolygon <- read.dbf("M:/geodata/project_data/11CAR/mupolygon_edits_fy16.dbf", as.is = TRUE)
...
corr <- read.csv("C:/workspace/report_correlation_fy2016_IA%25_WI%25_2016_09_06.csv")
...
qa_sub <- subset(qa, musym != new_musym)
...
if (dim(qa_project)[1] != 0) {
  kable(qa_project, caption = "Geodatabase changes missing from NASIS - Missing values (NA) need to be either added/removed from NASIS or the RTSD geodatabase, Otherwise they maybe splits that need to be relinked to a new additional mapunit.")
} else cat("SDJR Projects in NASIS match the Geodatabase")

if (dim(qa_geo)[1] != 0) {
  kable(qa_geo, caption = "NASIS changes missing from the Geodatabase - Missing values (NA) need to be either added/removed from NASIS or the RTSD geodatabase, Otherwise they maybe splits that need to be relinked to a new additional mapunit.")
...
outside <- corr_owners[!(corr_owners$Region %in% region), 
                  c("Region", "office", "areasymbol", "projectname", "new_musym", "musym", "n_polygons", "acres", "in_legend")]
