...
corr11 <- subset(corr, grepl(SS_Office, Office))
...
write.csv(corr_check, paste0("report_corr_check_", SS_Office, "_", format(Sys.time(), "%Y_%m_%d"), ".csv"))
...
```{r, checks}
# Checks
corr_check$ssa_musym <- paste0(corr_check$ssa_musym, "_", corr_check$old_musym_corr)
geo_check$ssa_musym <- paste0(geo_check$ssa_musym, "_", geo_check$old_musym_geo)


check1 <- join(corr_check, geo_check, by="ssa_musym", type = "full") # match geo_check to corr_check
check1_filter <- check1[apply(is.na(check1), 1, any), ]
kable(check1_filter) # Matches between the RTSD geodatabase to the NASIS correlation report

write.csv(check1, paste0("report_check1_", SS_Office, "_", format(Sys.time(), "%Y_%m_%d"), ".csv"))
write.csv(check1_filter, paste0("report_check1_filter_", SS_Office, "_", format(Sys.time(), "%Y_%m_%d"), ".csv"))
