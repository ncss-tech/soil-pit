...
if (nrow(na.exclude(lp.lo)) > 6) {
  bwplot(value ~ variable, data=lp.lo, ylab = "percent", main = "Boxplots of aggregate profile attributes")
...
if (length(na.exclude(lp.thk$thickness)) > 1){
  ...
  ylim <- c(max(lp.lo$value, na.rm = T), min(lp.lo$value, na.rm = T))
...
for (i in seq(times)){
  print(kable(hz_t[, c(TRUE, x == i)], align = "c", digits = 0, caption = "Horizon designations vs generic horizon designations (counts)"))
  }
...
n1 <- length(levels(lh.lo$genhz))
l <- levels(lh.lo$genhz)[n1:1]

bwplot(factor(lh.lo$genhz, levels = l) ~ value | variable, data = lh.lo, scales =list(x="free"), main = "Box plots of numeric variables by generic horizon", as.table = TRUE)
...
n1 <- length(levels(lh.lo$genhz))
n2 <- round(length(levels(lh.lo$variable))/4, 0)
l <- levels(lh.lo$genhz)[n1:1]

bwplot(factor(lh.lo$genhz, levels = l) ~ value | variable, data = lh.lo, scales=list(x="free"), main = "Box plots of numeric variables by generic horizon", as.table = TRUE, layout = c(4, n2))

# qplot(genhz, value, data = lh.lo[1:100, ], geom = "boxplot", main = "Boxplots of numeric variables by generic horizon") + facet_wrap(~variable, scales = "free", ncol = 4) + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
