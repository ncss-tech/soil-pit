...
idx <- pindex(length(l), 15)
  
for (i in unique(idx)){
  plot(l[idx == i], label = "upedonid")
  title("Soil profile plots")[1]
  }
```
...
idx <- pindex(dim(hz_t)[2],15)

for (i in unique(idx)){
  print(kable(hz_t[, c(TRUE, idx == i)], align = "c", digits = 0, caption = "Horizon designations vs generic horizon designations (counts)"))
  }
