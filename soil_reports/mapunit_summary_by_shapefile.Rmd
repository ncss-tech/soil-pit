...
shp <- "201511IND057_Cslm,WLwln,6t12ps,se_QA"
...
mlra_i <- intersect(mupolygon, mlra)
mlra_df <- data.frame(mlra_i, stringsAsFactors = F)
mlra_df$Acres <- gArea(mlra_i, byid = TRUE) * 0.000247105
mlra_acres <- ddply(mlra_df, .(mlra = MLRARSYM), summarise, acres = sum(Acres))
...
bwplot(SSA_MUSYM ~ value | variable[length(pol.lo1$variable):1], data = pol.lo1, scales = list(x="free"), main = "Boxplots of polygon metrics", as.table = TRUE)
...
bwplot(factor(SSA_MUSYM, levels=levels(temp)[length(levels(temp)):1]) ~ value|variable, data=data.lo, scales=list(x="free"), main = "Boxplots of map unit properties", as.table = TRUE, layout = c(2, 4))
