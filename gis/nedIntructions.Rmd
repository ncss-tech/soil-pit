...
```{r, eval=FALSE}
# Set parameters
source("C:/Users/Stephen/SkyDrive/R/gis/nedFunctions.R")
source("C:/Users/Stephen/SkyDrive/R/gis/gdalUtilsFunctions.R")
tile.p <- "C:/geodata/elevation/ned/tiles/img/"
office.l <- c("ATL", "AUR", "CAR", "CLI", "FIN", "GAL", "IND", "JUN", "SPR", "UNI", "WAV")
pd.p <- "E:/geodata/project_data/11"
geodatabase <- paste(pd.p, office.l,"/11", office.l, ".gdb", sep="")
nlcd.p <- "D:/geodata/land_use_land_cover/nlcd_2011_landcover_2011_edition_2014_03_31.img"
nlcd.p.l <- paste(pd.p, office.l,"/nlcd", 30, "m_11", office.l, "_lulc2011.tif", sep="")
crsarg <- "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
```
...
```{r, eval=FALSE}
ned.l<- makeNedList(geodatabase, office.l)
...
dst.p <- paste(pd.p, office.l[i], "/ned09d_11", office.l[i], ".tif", sep="")

mosaicList(mo.l, dst.p)
```
...
```{r, eval=FALSE}
dem.l <- paste(pd.p, office.l, "/ned09d_11", office.l, ".tif", sep="")

batchWarp(dem.l, nlcd.p, "09d", "10m", "bilinear", crsarg)

dem.l <- paste(pd.p, office.l, "/ned10m_11", office.l, ".tif", sep="")

batchWarpAverage(dem.l, "10m", "30m")

dem30.tif <- paste(pd.p, office.l, "/ned30m_11", office.l, ".tif", sep="")
```
...
```{r, eval=FALSE}
nlcd.l <- list(nlcd.p)
dst.p <- "C:/geodata/project_data/REGION11/nlcd30m_R11.tif"
mosaicNlcdList(nlcd.l, dst.p)

dem.l <- list(paste(pd.p, office.l, "/ned30m_11", office.l, ".tif", sep=""))
dem.p <- "C:/geodata/project_data/Region11/ned30m_R11.tif"
