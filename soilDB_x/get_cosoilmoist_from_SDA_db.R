get_cosoimoist_from_SDA_db <- function(nationalmusym, impute = TRUE){
  q.cosoilmoist <- paste0("SELECT mu.nationalmusym, mu.mukey, mu.muname, c.cokey, c.compname, c.comppct_r, month, flodfreqcl, pondfreqcl, cosoilmoistkey, soimoistdept_l, soimoistdept_r, soimoistdept_h, soimoistdepb_l, soimoistdepb_r, soimoistdepb_h, soimoiststat
  ...
  WHERE nationalmusym IN ('", paste0(nationalmusym, collapse = "', '"), "')
  ...
  # Convert to factors
  freqcl <- c("None", "Very Rare", "Rare", "Occassional", "Frequent", "Very Frequent")
