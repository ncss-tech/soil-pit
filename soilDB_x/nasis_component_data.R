...
## just the components
get_component_data_from_NASIS_db <- function(minColumns = TRUE) {
  ...
  q <- ifelse(minColumns == TRUE, paste(select_min, from), paste(select_max, from))
...
get_component_horizon_data_from_NASIS_db <- function(minColumns = TRUE) {
  # must have RODBC installed
  if(!requireNamespace('RODBC'))
    stop('please install the `RODBC` package', call.=FALSE)
  ...
  from <- "FROM chorizon_View_1 ch
  
  LEFT OUTER JOIN (SELECT chiidref, texture FROM chtexturegrp_View_1 WHERE rvindicator = 'TRUE') AS cht ON cht.chiidref = ch.chiid
...
fetchNASIS_component_data <- function(rmHzErrors=TRUE, minColumns = TRUE) {
  # must have RODBC installed
  if(!requireNamespace('RODBC'))
    stop('please install the `RODBC` package', call.=FALSE)
