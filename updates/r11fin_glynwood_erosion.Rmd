...
The primary objective is to resolve county joins between several Glynwood B-slope map units with different erosion phases. In order to achieve this objective, two approaches were evaluated. The first looked at the purpose of this project are to evaluate the One of the goals of this projects was to develop an ArcSIE model in order to differentiate erosion classes. The analysis below is intended to evaluate the field data and ArcSIE model.
...
data_sub$natmuSsaEro <-  with(data_sub, paste0(nationalmu, "-", AREASYMBOL, "-", EroClassNASIS))

test <- with(data_sub, table(natmuSsaEro, EroClassFD))
test
