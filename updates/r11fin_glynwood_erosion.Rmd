...
data$soilname2 <- with(data, paste0(soilname, ifelse(soilname == "Glynwood", paste0("-", as.character(EroClassFD)), "")))

data_sp <- data
coordinates(data_sp) <- ~ long + lat
proj4string(data_sp) <- CRS("+init=epsg:4326")
data_sp2 <- spTransform(data_sp, CRS(proj4string(gw_pol)))
# writeOGR(data_sp2, dsn = ownCloud, layer = "glynwood_bslopes", driver = "ESRI Shapefile")
...
write.csv(data, file = paste0(ownCloud, "Pts_gnbero_27Jan17_geodata.csv"))
```

# Evaluate the Project by AREASYMBOL

```{r}
data <- read.csv("Pts_gnbero_27Jan17_geodata.csv")
