...
# Region 11 files
fd <- paste0(geodata, "project_data/11IND/")
...
test3 <- cforest(as.factor(soilname3) ~ maxcrv + mincrv + PlanCrv + ProfCrv + SlopeSIE + relpos + wetness + elev + slope5 + kt5 + kp5 + wetness2 + mvalley + z2streams, data = gw)
varimp(test3)
confusionMatrix(data = predict(test3, type = "response", OOB = TRUE), reference = gw$soilname3, positive = "TRUE")

gw$gw_severe <- ifelse(gw$soilname3 == "Glynwood-severe", TRUE, FALSE)
test3 <- randomForest(as.factor(gw_severe) ~ elev + slope5 + kt5 + kp5 + wetness2 + mvalley + z2streams, data = gw)
confusionMatrix(data = predict(test3, type = "prob", OOB = TRUE)[, 2] > 0.4, reference = gw$gw_severe, positive = "TRUE")

test3 <- glm(as.factor(gw_severe) ~ elev + PlanCrv + ProfCrv + SlopeSIE + wetness2 + mvalley + z2streams, data = gw, family = "binomial")
