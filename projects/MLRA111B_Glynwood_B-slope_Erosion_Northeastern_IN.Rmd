...
new <- read_sf(dsn = paste0(ownCloud, "r11_fin_spatial_new.shp"), layer = "r11_fin_spatial_new")
...
test <- project_sub[, c("musym", "muname")]
new_df2 <- merge(new_df,test, by = "musym", all.x = TRUE)
...
test <- test[order(test$areasymbol), ]

fin_int <- st_intersection(new, old)

test2 <- group_by(fin_int, areasymbol_new, musym_new, musym) %>% summarize(
  acres_new = round(sum(shape_area_new * 0.000247)),
  acres_old = round(sum(shape_area * 0.000247)),
  diff = acres_old - acres_new
)

by(fin_int, with(fin_)
```
