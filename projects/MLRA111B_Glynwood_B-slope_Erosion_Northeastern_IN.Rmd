...
project <- get_projectmapunit_from_NASIS()
mukeys <- unique(project$mukey)
idx <- with(project, is.na(areasymbol))
project_sub <- project[idx, ]
# add musym to new mlra mapunits in Excel
# write.csv(project_sub, file = paste0(ownCloud, "r11_cor_legend.csv"), row.names = FALSE)
project_sub <- read.csv(paste0(ownCloud, "r11_cor_legend.csv"))
project2 <- rbind(project[!idx, ],
                  project_sub
                  )


old <- read_sf(dsn = paste0(ownCloud, "r11_fin_spatial_old.shp"), layer = "r11_fin_spatial_old")
st_crs(old) <- "+init=epsg:5070"
# old_sub <- old[old$mukey %in% mukeys,]
# rm(old)
names(old) <- tolower(names(old))


new1 <- read_sf(dsn = paste0(ownCloud, "r11_fin_spatial_new.shp"), layer = "r11_fin_spatial_new")
new <- read_sf(dsn = paste0(ownCloud, "r11_fin_spatial_new2.shp"), layer = "r11_fin_spatial_new2")
st_crs(new) <- "+init=epsg:5070"
# Relabel Column Names
# idx <- ! names(new) %in% "geometry"
# names(new)[idx] <- paste0(names(new)[idx], "_new")
# idx <- with(new, MUSYM != ORIG_MUSYM | !is.na(Creator_Field) | Editor_Field != "ALENA.STEPHENS"
# new_sub <- new[idx, ]
names(new) <- tolower(names(new))
new_df <- as.data.frame(new)
new_df$geometry <- NULL
test1 <- project_sub[, c("musym", "nationalmusym", "muname")]
new_df2 <- merge(new_df, test1, by = "musym", all.x = TRUE)
new_df2 <- merge(new_df2, project[-idx, c("areasymbol", "musym", "nationalmusym", "muname")], by = c("areasymbol", "musym"), all.x = TRUE)
new_df2 <- within(new_df2, {
  muname = ifelse(is.na(muname), muname.y, muname)
  nationalmusym = ifelse(is.na(nationalmusym.y), nationalmusym.x, nationalmusym.y)
  })

vars <- c("areasymbol", "musym")
test <- by(new_df2, new_df2[vars], function(x) data.frame(
  x[1, c(vars, "nationalmusym", "muname")],
  acres = round(sum(x$shape_area * 0.000247))
  ))
test <- do.call("rbind", test)
test <- test[with(test, order(areasymbol, musym)), ]
...
# Intersect new and old MUPOLYGONs
fin_int <- st_intersection(new, old)
fin_df <- as.data.frame(fin_int)


# Summarize Interaction Acres
vars <- c("areasymbol_new", "musym_new", "musym")
fin_df$idx <- apply(fin_df[vars], 1, paste0, collapse = "")
test2 <- by(fin_df, fin_df$idx, function(x) {
  acres_new = round(sum(x$shape_area_new * 0.000247))
  acres_old = round(sum(x$shape_area * 0.000247))
  
  data.frame(x[vars][1, ],
             acres_new,
             acres_old,
             diff = acres_old - acres_new
             )})
test2 <- do.call("rbind", test2)
