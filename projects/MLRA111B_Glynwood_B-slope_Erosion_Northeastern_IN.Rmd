...
```{r search NASIS for extra pedons, eval=FALSE, echo=FALSE}
...
# all 11-FIN MUPOLYGONS exported from geodatabase
R11FIN_pol <- read_sf(dsn = "M:/geodata/soils/2017/MLRA_11_FIN_FY17.gdb", layer = "MUPOLYGON")
st_crs(R11FIN_pol) <- st_crs("+init=epsg:5070")

ssa <- read_sf(dsn = "M:/geodata/soils/2017/MLRA_11_FIN_FY17.gdb", layer = "SAPOLYGON")
st_crs(ssa) <- st_crs("+init=epsg:5070")
ssa <- ssa[ssa$AREASYMBOL %in% project$areasymbol, ]
ssa <- st_transform(ssa, crs = "+init=epsg:4326")


# NASIS site data
test <- get_site_data_from_NASIS_db()
test <- test[complete.cases(test[c("x", "y")]), ]

test_sp <- test
coordinates(test_sp) <- ~ x + y
proj4string(test_sp) <- CRS("+init=epsg:4326")

test_sf <- st_as_sf(test_sp)
test_sf <- st_transform(test_sf, crs = "+init=epsg:5070")


# filter mupolygons by mukey
R11FIN_pol_sub <- R11FIN_pol[R11FIN_pol$MUKEY %in% project$mukey, ]

# intersect mupolygons and points
R11FIN_pol_sub2 <- R11FIN_pol[R11FIN_pol$AREASYMBOL %in% project$areasymbol, ]
idx <- st_intersects(test_sf, R11FIN_pol_sub2)
idx2 <- lapply(idx, function(x) ifelse(length(x) == 0, NA, x))
R11FIN_pol_int <- R11FIN_pol_sub2[unlist(idx2), ]
R11FIN_pol_int$upedonid <- test_sf$pedon_id

# save and export files
write_sf(R11FIN_pol_sub, 
         dsn = paste0(ownCloud, "201711FIN001_glynwood_mupolygon.shp"), 
         layer = "201711FIN001_glynwood_mupolygon", 
         driver = "ESRI Shapefile"
         )
save(R11FIN_pol_int, R11FIN_pol_sub, ssa, gw_series, file = paste0(ownCloud, "201711FIN001_glynwood_pol.RData"))
```
