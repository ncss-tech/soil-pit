...
R11FIN_pol <- read_sf(dsn = "C:/workspace", layer = "R11FIN_pol")
...
test_sp <- spTransform(test_sp, CRS("+init=epsg:5070"))
test_sf <- st_as_sf(test_sp)
st_crs(test_sf) <-  st_crs(R11FIN_pol)

# intersect mupolygons and points
idx <- st_intersects(test_sf, R11FIN_pol)
R11FIN_gw_pol_sub <- R11FIN_pol[unlist(idx), ]

save(R11FIN_gw_pol_sub, file = paste0(ownCloud, "R11FIN_gw_pol_sub.RData"))
```

```{r nasis pedons, eval=TRUE}
...
gw_pol <- read_sf(dsn = paste0(ownCloud, "glynwood.shp"), layer = "glynwood")
...
save(gw_pol, ssa, gw_series, file = paste0(ownCloud, "201711FIN_Glynwood_pol.RData"))
```


```{r import data and tranform, eval=TRUE}
load(file = paste0(ownCloud, "201711FIN_Glynwood_pol.RData"))
