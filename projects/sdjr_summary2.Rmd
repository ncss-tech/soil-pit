...
prg2 <- prg %>%
  mutate(region = gsub("-", "", substr(mlrassoarea, 1, 2))) %>%
  filter(region == ro & ! projecttypename %in% c("ES", "ALL PROJECTS"))
...
prg3 <- select(prg2, mlrassoarea, projecttypename, fiscalyear, acre_progress)
prg4 <- rbind(prg3, r11)

ggplot(prg4, aes(x = fiscalyear, y = acre_progress, color = projecttypename)) + 
  geom_line(lwd = 1) +
  geom_point(cex = 2) +
