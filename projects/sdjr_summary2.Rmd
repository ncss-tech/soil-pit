...
ownCloud <- "C:/Users/stephen/ownCloud/data/lims_data/"
ro <- 11
...
r11 <- filter(regions, region == 11 & projecttypename == "ES")[-1]
r11_sso <- filter(prg_sub, region == 11 & projecttypename == "ES") %>% 
  select(mlrassoarea, projecttypename, fiscalyear, n_project, n_areasymbol, n_nationalmusym, n_spatial, acre_progress)
r11_all <- rbind(r11, r11_sso)
r11_all$n_project_t <- cut(r11_all$n_project,
                           breaks = c(0, 1, 5, 15, 35, 50, 100),
                           labels = c("1", "5", "15", "35", "50", "100+")
                           )

# plot of region 11 acres
...
filter(r11_all, mlrassoarea != 11) %>%
ggplot(aes(x = n_nationalmusym, y = n_project, color = mlrassoarea)) + 
  geom_point(cex = 5) +
  facet_wrap(~ paste(projecttypename, "-", fiscalyear), scales = "free", ncol = 4) +
...
ggplot(aes(x = n_nationalmusym, y = n_project, shape = region)) + #, size = acre_progress)) + 
  geom_point(size = 3) +
  facet_wrap(~ paste(projecttypename, "-", fiscalyear), scale = "free") +
  scale_shape_manual(values = 1:12) +
  theme(aspect.ratio = 1) +
  ggtitle("SSD Projects")


# plot of all regions number of spatial changes
filter(regions, projecttypename %in% c("SDJR (Obsolete)", "EVAL", "MLRA")) %>%
ggplot(aes(x = n_spatial, y = n_areasymbol, shape = region)) + 
  geom_point(cex = 3) +
