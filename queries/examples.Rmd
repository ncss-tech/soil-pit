...
get_sdjr_NASIS_db <- function(selected_set = TRUE) {
  ...
  q <- paste("SELECT p.uprojectid, p.projectname, a.areasymbol, lmu.lmapunitiid AS mukey, lmu.musym, mu.nationalmusym, muacres
  
  FROM",
  if (selected_set != TRUE) {
  ...
  WHERE a.areasymbol LIKE '11-%' AND p.projectname LIKE 'SDJR%' AND p.uprojectid LIKE '2016%'
      AND mu.mutype != 1 AND p.projectapprovedflag = 1
