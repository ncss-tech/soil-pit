...
f@horizons$d_hue <- d_munsell$hue
f@horizons$d_value <- d_munsell$value
f@horizons$d_chroma <- d_munsell$chroma
f@horizons$m_hue <- m_munsell$hue
f@horizons$m_value <- m_munsell$value
f@horizons$m_chroma <- d_munsell$chroma
...
if(length(na.exclude(is.na(f@horizons$genhz)))){
print(xtable(addmargins(table(f@horizons$genhz, f@horizons$hzname)), digits=0))
}
...
print(xtable(addmargins(table(h$genhz, h$texture_class)), digits=0))
}

# Tex Mod & Class by generic horizon
if(length(na.exclude(f@horizons$genhz))>0){
print(xtable(addmargins(table(h$genhz, h$texture)), digits=0))
}

# Dry hue by generic horizon
if(length(na.exclude(f@horizons$genhz))>0){
print(xtable(addmargins(table(h$genhz, h$d_hue)), digits=0))
}

# Moist hue by generic horizon
if(length(na.exclude(f@horizons$genhz))>0){
print(xtable(addmargins(table(h$genhz, h$m_hue)), digits=0))
}

# Effervescence by generic horizon
if(length(na.exclude(f@horizons$genhz))>0){
print(xtable(addmargins(table(h$genhz, h$effervescence)), digits=0))
}
...
print(xtable(cast(d.cs, diag_kind ~ variable, value='range'), digits=0))
