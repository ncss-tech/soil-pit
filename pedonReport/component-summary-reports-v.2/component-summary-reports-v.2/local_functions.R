...
# this is commonly used to  geomorphic information, 
# but can be used with any user-specified data + levels
categorical.prop.table <- function(x, l=c("Toeslope", "Footslope", "Backslope", "Shoulder", "Summit"), digits=2) {
  # convert into factor, with user-defined levels
  x <- factor(x, levels=l)
...
summarise.pedon.gis.data <- function(d) {
  ...
  # TODO: this is a hack remove gis_aspect if present
  d$gis_aspect <- NULL
