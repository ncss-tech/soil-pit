...
test <- 1:n
interval <- 4
x <- ceiling(n/interval)
b <- n - (x - floor(n/interval))

for (i in 0:(x-1)){
  if (i < (x-1)){
    print(kable(lh.c[c(1, i*interval+(2:5))], align = "c"))
    } else {print(kable(lh.c[c(1, b:n)], align = "c"))}
}
...
bwplot(factor(genhz, levels=levels(lh$genhz)[length(levels(lh$genhz)):1]) ~ value|variable, data = rbind(lh.lo), scales=list(x="free"), main = "Box plots of numeric variables by generic horizon")          
```
