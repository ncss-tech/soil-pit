...
if(!all(is.na(s[, c("shapedown", "shapeacross")]))) {
  s$shapedown <- as.factor(s$shapedown)
  levels(s$shapedown) <- c("Convex", "Linear", "Concave")
  s$shapeacross <- as.factor(s$shapeacross)
  levels(s$shapeacross) <- c("Convex", "Linear", "Concave")
  print(xtable(addmargins(table(s$shapedown, s$shapeacross)), digits=0, align=rep("c", 5), caption="Slope shape"), type="html", caption.placement="top")
