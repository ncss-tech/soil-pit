...
na_replace <- function(x){
  if(class(x) == "character" | class(x) == "logical") {x <- replace(x, is.na(x), "missing")} 
  else (x <-  x)
}
...
if(!all(is.na(s$aspect_field))) {
  ...
  kable(cbind(morf.wi, aspect.wi["aspect_field"]), caption="Elevation, slope gradient and aspect")
} else(kable(morf.wi, caption="Elevation and slope gradient", align = "l")
)         
...
if(!all(is.na(s[, c("pmorigin", "pmkind", "landform.string")]))) {
  ...
  kable(pm.lf, caption="Parent material vs landform")}
...
if(!all(is.na(s[, c("shapedown", "shapeacross")]))) {
  ...
  kable(addmargins(table(s$shapedown, s$shapeacross)), caption = "Down slope (y-axis) vs across slope (x-axis)")
} 
...
if(!all(is.na(s[, c("hillslope_pos", "drainagecl")]))) {
  ...
  kable(addmargins(table(s$drainagecl, s$hillslope_pos)), digits = 0, caption = "Drainage class vs hillslope position")
}
```
...
```{r, echo=FALSE, results='asis'}
kable(addmargins(table(h$genhz, h$hzname)), digits = 0) 
```
...
bwplot(factor(genhz, levels = levels(h$genhz)[length(levels(h$genhz)):1]) ~ value|variable, data = rbind(h.lo1, h.lo2), scales = list(x="free"), main = "Box plots of numeric variables by generic horizon")          
```
...
```{r, echo=FALSE, results='asis'}
kable(addmargins(table(h$genhz, h$texture_class)), digits = 0)
```
...
```{r, echo=FALSE, results='asis'}
kable(addmargins(table(h$genhz, h$texture)), digits = 0)
```
...
```{r, echo=FALSE, results='asis'}
kable(addmargins(table(h$genhz, h$d_hue)), digits = 0)
```
...
```{r, echo=FALSE, results='asis'}
kable(addmargins(table(h$genhz, h$m_hue)), digits = 0)
```
...
kable(addmargins(table(h$genhz, h$effervescence)), digits = 0)
```
...
kable(cast(genhz.5n, genhz ~ variable, value = 'range'), align = "c")

bwplot(factor(genhz, levels=levels(h$genhz)[length(levels(h$genhz)):1]) ~ value|variable, data = genhz.lo, scales = list(x = "free"), xlab = "cm", horizontal = TRUE, main = "Boxplot of generic horizon thicknesses")          
```
