...
h$genhz <- generalize.hz(h$hzname, ghr$Crosby$n, ghr$Crosby$p)
...
temp <- s
temp <- temp[complete.cases(c(temp$x_std)),]
temp <- temp[complete.cases(c(temp$y_std)),]
coordinates(temp) <- ~x_std+y_std
plot(temp, pch=16)
map("county", add=T)
