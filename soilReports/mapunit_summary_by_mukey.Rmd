...
data$MUKEY_AS_MUSYM <- as.character(paste(data$MUKEY, data$AREASYMBOL, data$MUSYM))
data2 <- data
data2$MUKEY <- "representative"
data2$MUKEY_AS_MUSYM <- "representative"
...
print(xtable(prop.table(xtabs(~MUKEY_AS_MUSYM+slopeBreaks,data=data), margin=1)*100, digits=0, caption="Slope breaks"), type="html", caption.placement="top")
print(xtable(prop.table(xtabs(~MUKEY_AS_MUSYM+aspectBreaks,data=data), margin=1)*100, digits=0, caption="Aspect breaks"), type="html", caption.placement="top")
print(xtable(prop.table(xtabs(~MUKEY_AS_MUSYM+valleysBreaks,data=data), margin=1)*100, digits=0, caption="Upland vs. lowland"), type="html", caption.placement="top")
```

## Percentiles
### *(e.g. min, 25th, median, 75th, max)*
```{r quantiles by mukey, echo=FALSE, results="asis"}
data.m <- melt(data, id.vars="MUKEY_AS_MUSYM", measure.vars=c("elev", "slope", "valleys", "wetness", "relief", "ppt", "temp", "ffd"))
data.cs <- ddply(data.m, .(variable, MUKEY_AS_MUSYM), .fun=conditional.l.rv.h.summary)
data.cs.c <- cast(data.cs, MUKEY_AS_MUSYM ~ variable, value='range')
data.n <- ddply(data, .(MUKEY_AS_MUSYM), .drop=T, summarize, n=length(MUKEY))
print(xtable(merge(data.cs.c, data.n, by="MUKEY_AS_MUSYM")), type="html")
aspect.m <- melt(data, id.vars="MUKEY_AS_MUSYM", measure.vars=c("aspect"))
aspect.m$value <- circular(aspect.m$value, template="geographic", units="degrees", modulo="2pi")
aspect.cs <- ddply(aspect.m, .(variable, MUKEY_AS_MUSYM), .fun=conditional.l.rv.h.summary)
aspect.cs.c <- cast(aspect.cs, MUKEY_AS_MUSYM ~ variable, value='range')
print(xtable(merge(aspect.cs.c, data.n, by="MUKEY_AS_MUSYM")), type="html")
```

## Boxplots of map unit properties
```{r bwplot of map unit properties, echo=FALSE}
### *(min, 5th, 25th, median, 75th, 95th, max)*
bwplot(MUKEY_AS_MUSYM ~ value|variable, data=data.m, scales=list(x="free"))          
```
