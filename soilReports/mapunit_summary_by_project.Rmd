...
project <- c('SDJR - MLRA 111A - Crosby-Miami silt loams, 0 to 6 percent slopes') # List of project
...
h <- 2+length(levels(data$AREASYMBOL))*.75
...
pol.m <- melt(pol, id.vars="SSA_MUSYM", measure.vars=c("Acres", "Circularity"))
pol.as.m <- melt(pol, id.vars=c("AREASYMBOL", "MUSYM"), measure.vars=c("Acres"))
pol.as.cs1 <- ddply(pol.as.m, .(AREASYMBOL, MUSYM, variable), .fun=conditional.l.rv.h.summary)
...
print(xtable(pol.as.me, digits=0, caption="Summary of MUSYM by AREASYMBOL"), type="html", caption.placement="top")
```
...
```{r bwplot of polygon metrics, echo=FALSE, fig.height=h}
bwplot(SSA_MUSYM ~ value|variable, data=pol.m, scales=list(x="free"))
```
...
print(xtable(prop.table(xtabs(~SSA_MUSYM+slopeBreaks,data=data), margin=1)*100, digits=0, caption="Slope breaks"), type="html", caption.placement="top")
print(xtable(prop.table(xtabs(~SSA_MUSYM+aspectBreaks,data=data), margin=1)*100, digits=0, caption="Aspect breaks"), type="html", caption.placement="top")
print(xtable(prop.table(xtabs(~SSA_MUSYM+valleyBreaks,data=data), margin=1)*100, digits=0, caption="Upland vs. lowland"), type="html", caption.placement="top")
print(xtable(prop.table(xtabs(~SSA_MUSYM+lulcBreaks,data=data), margin=1)*100, digits=0, caption="Landuse and Landcover"), type="html", caption.placement="top")
```
...
data.m <- melt(data, id.vars="SSA_MUSYM", measure.vars=c("elev", "slope", "valley", "wetness", "relief", "ppt", "temp", "ffp"))
data.cs <- ddply(data.m, .(variable, SSA_MUSYM), .fun=conditional.l.rv.h.summary)
data.cs.c <- cast(data.cs, SSA_MUSYM ~ variable, value='range')
data.n <- ddply(data, .(SSA_MUSYM), .drop=T, summarize, n=length(SSA_MUSYM))
print(xtable(merge(data.cs.c, data.n, by="SSA_MUSYM")), type="html")
aspect.m <- melt(data, id.vars="SSA_MUSYM", measure.vars=c("aspect"))
aspect.m$value <- circular(aspect.m$value, template="geographic", units="degrees", modulo="2pi")
aspect.cs <- ddply(aspect.m, .(variable, SSA_MUSYM), .fun=conditional.l.rv.h.summary)
aspect.cs.c <- cast(aspect.cs, SSA_MUSYM ~ variable, value='range')
print(xtable(merge(aspect.cs.c, data.n, by="SSA_MUSYM")), type="html")
```

## Boxplots of map unit properties
Graphical five number summary plus outliers (outliers, 5th, 25th, median, 75th, 95th, outliers)(percentiles) 

```{r bwplot of map unit properties, echo=FALSE, fig.height=h}
### *(min, 5th, 25th, median, 75th, 95th, max)*
temp <- factor(unique(data$SSA_MUSYM))

bwplot(factor(SSA_MUSYM, levels=levels(temp)[length(levels(temp)):1]) ~ value|variable, data=data.m, scales=list(x="free"))          
```
