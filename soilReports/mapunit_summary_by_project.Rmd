...
pol.5n2 <- ddply(pol, .(SSA_MUSYM), summarize, nPolygons=length(MUSYM), nArces=round(sum(Acres), 0))
pol.5n <- merge(pol.5n1, pol.5n2, by=c("SSA_MUSYM"))
print(xtable(pol.5n, digits=0, caption="Summary of MUSYM by AREASYMBOL"), type="html", caption.placement="top")
```
...
data.5n.c <- cast(data.5n, SSA_MUSYM ~ variable, value='range')
data.n <- ddply(data, .(SSA_MUSYM), .drop=T, summarize, n=length(SSA_MUSYM))
print(xtable(merge(data.5n.c, data.n, by="SSA_MUSYM"), align=rep("c", 2+ncol(data.5n.c))), type="html")
...
aspect.5n.c <- cast(aspect.5n, SSA_MUSYM ~ variable, value='range')
print(xtable(merge(aspect.5n.c, data.n, by="SSA_MUSYM")), type="html")
```
