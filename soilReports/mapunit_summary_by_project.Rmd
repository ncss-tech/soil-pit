...
project <- c("SDJR - MLRA 111A - Crosby-Lewisburg silt loams, 0 to 2 percent slopes") # name of project
...
kable(prop.table(xtabs(~ SSA_MUSYM + slopeBreaks, data = data), margin = 1) * 100, align = "c", digits=0, caption="Slope breaks")
kable(prop.table(xtabs(~ SSA_MUSYM + aspectBreaks, data = data), margin = 1) * 100, align = "c", digits = 0, caption = "Aspect breaks")
kable(prop.table(xtabs(~ SSA_MUSYM + valleyBreaks, data = data), margin = 1) * 100, align = "c", digits = 0, caption = "Upland vs. lowland")
kable(addmargins(prop.table(xtabs(~ SSA_MUSYM + lulcBreaks, data = data), margin = 1) * 100), digits = 0, align = "c", caption = "Landuse and Landcover")
```

## Quantile breaks
Five number summary (min, 25th, median, 75th, max)(percentiles) and number of random samples (n)

```{r quantiles by musym, echo=FALSE, results="asis", fig.height=nh}
